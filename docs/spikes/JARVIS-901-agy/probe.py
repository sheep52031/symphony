...
def _valid_terminal(payload: object) -> bool:
    if not isinstance(payload, dict) or payload.get("status") not in TERMINAL_STATUSES:
        return False
    turns = payload.get("num_turns")
    return isinstance(turns, int) and not isinstance(turns, bool) and turns >= 0
...
def run_fixture(
    ...
    *,
    ...
    def consume_line(name: str, line: bytes) -> None:
        nonlocal first_token_seen, terminal_at
        ...
        if payload is not None:
            if event.get("event") != "result" or not _valid_terminal(payload):
                result.invalid_terminal_results += 1
            else:
    ...
    while True:
        ...
        try:
            ...
            if process.poll() is not None and streams_closed == {"stdout", "stderr"}:
                break
            if stop_started_at is not None and now - stop_started_at >= cleanup_grace * 3:
                break
            continue
        if chunk is None:
